...
class Admin_page extends MY_Controller
{
    ...
    {
        $page_url = $this->get_page_url($action, array(), true);
    ...
    protected function get_globals()
    {
        ...
        $globals['user'] = $globals['menus'] = $globals['leaves'] = array();
        if ($this->is_authed()) {
    ...
    protected function initialize()
    {
        ...
        $login_action = $this->get_page_url('login', array(), true);
    ...
    protected function get_menus($role_id, $is_revoked = 0)
    {
        ...
        $where = array('parent_id' => $branch_ids, 'is_removed' => 0);
        $rows = $this->menu_model->parse_where($where)->all();
        $leaves = array_fill_keys($branch_ids, array());
        foreach ($rows as $row) {
    ...
    protected function role_has_privilege($role_id, $is_revoked = 0, $menu_id = 0, $operation = 'all')
    {
        ...
        $rows = $this->menu_model->get_menu_rows(array($menu_id));
        $menu_ids = array_column($rows, 'id');
        array_unshift($menu_ids, 0);
        $priv_ids = array();
        $where = array('menu_id' => $menu_ids, 'operation' => $operation, 'is_removed' => 0);
